...
// บันทึกค่าธรรมเนียม OPN ลง accounting_expenses อัตโนมัติ
async function recordPaymentFee(db, { chargeId, packageKey, amount, method, userId = null }) {
  const FEE_RATE = { card: 0.0365, promptpay: 0.015 };
  const rate       = FEE_RATE[method] || 0.015;
  const amountNum  = parseFloat(amount) || 0;
  const fee        = Math.round(amountNum * rate * 100) / 100;
  const today      = new Date().toISOString().split('T')[0];
  const methodLabel = method === 'card' ? 'บัตรเครดิต (3.65%)' : 'PromptPay (1.5%)';
  console.log(`[fee] ${method} ${packageKey} amount=${amountNum} fee=${fee} chargeId=${chargeId} userId=${userId}`);
  try {
    await db.query(
      ...
      [
        'payment',
        `ค่าธรรมเนียม OPN ${methodLabel} — ${packageKey} — ${chargeId}`,
        fee,
        today,
        userId,
      ]
    );
    console.log(`[fee] recorded ฿${fee} to accounting_expenses`);
  } catch (e) { console.error('[fee] recordPaymentFee error:', e.message); }
}
...
router.post('/charge', authMiddleware, async (req, res) => {
  try {
    ...
    await recordPaymentFee(db, { chargeId: charge.id, packageKey: pkg.key, amount: pkg.price, method: 'card', userId: req.user.id });
...
router.post('/webhook/opn', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    ...
    // บันทึกค่าธรรมเนียม PromptPay 1.5% ลงระบบบัญชี
    await recordPaymentFee(db, { chargeId: charge.id, packageKey: pr.package_key, amount: pr.amount, method: 'promptpay', userId: pr.user_id });
...
router.post('/test/simulate-payment', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await recordPaymentFee(db, { chargeId: charge_id, packageKey: pr.package_key, amount: pr.amount, method: 'promptpay', userId: pr.user_id });
