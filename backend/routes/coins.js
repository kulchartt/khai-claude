...
async function recordPaymentFee(db, { chargeId, packageKey, amount, method }) {
  ...
  console.log(`[fee] ${method} ${packageKey} amount=${amountNum} fee=${fee} chargeId=${chargeId}`);
  try {
    await db.query(
      `INSERT INTO accounting_expenses (category, description, amount, expense_date, created_by)
       VALUES ($1, $2, $3, $4, NULL)`,
      [
        'payment',   // ตรงกับ EXPENSE_CATS key
...
router.post('/webhook/opn', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    ...
    await recordPaymentFee(db, { chargeId: charge.id, packageKey: pr.package_key, amount: pr.amount, method: 'promptpay' });
...
router.post('/test/simulate-payment', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await recordPaymentFee(db, { chargeId: charge_id, packageKey: pr.package_key, amount: pr.amount, method: 'promptpay' });
