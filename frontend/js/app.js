    ...
    c.innerHTML=`<div style="margin-top:16px">
      ...
      ${fbs.map(f=>`
        <div style="border:1px solid var(--border);border-left:3px solid ${fbStatusColor[f.status||'new']};border-radius:var(--radius);padding:14px;margin-bottom:10px;background:var(--card-bg)">
          ...
          <div style="font-size:14px;color:var(--text);line-height:1.6;margin-bottom:${f.admin_reply?'10px':'0'}">${f.message}</div>
          ${f.admin_reply?`<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:var(--radius);padding:10px 12px;font-size:13px"><span style="font-weight:600;color:#16a34a">💬 ตอบกลับจากแอดมิน:</span><div style="margin-top:4px;color:var(--text);line-height:1.5">${f.admin_reply}</div></div>`:''}
        </div>`).join('')}
    </div>`;
