console.log('%c app.js v20260417r10 ', 'background:#16a34a;color:#fff;padding:2px 6px;border-radius:4px');
// Feature flags — เปิด/ปิด feature ตาม section
const FEATURES = {
  LIVE: false,         // ปิดไว้ก่อน — เปิดใช้ในตลาดมือหนึ่ง
  HOLIDAY_MODE: false, // ปิดไว้ก่อน — ยังไม่ block buyer จริง + ไม่มี auto re-open
  RESERVATION: false,  // ปิดไว้ก่อน — ใช้ chat จอง FB Marketplace style แทน
};
      ...
      <div class="detail-hero">
        ...
        <div class="detail-info">
          ...
          <div class="detail-actions">
            ${FEATURES.RESERVATION&&!isOwner&&p.status==='available'&&!p.seller_holiday_mode?`<button class="btn btn-reserve" onclick="doReserve(${p.id})">🔖 จอง</button>`:''}
...
async function openCart(){
  if(!state.user){openOverlay('loginOverlay');return;}
  try{const items=await api.getCart();const c=document.getElementById('cartContent');if(!items.length){c.innerHTML='<div class="empty-msg">ตะกร้าว่างเปล่า<br><br><button class="btn btn-g" onclick="goPage(\'home\')">ดูสินค้า</button></div>';goPage('cart');return;}const total=items.reduce((s,x)=>s+x.price*x.qty,0);c.innerHTML=items.map(x=>{const lockEnd=x.cart_locked_until?new Date(x.cart_locked_until):null;const lockHtml=lockEnd&&lockEnd>new Date()?`<div class="lock-timer" data-lock-end="${x.cart_locked_until}" style="font-size:11px;color:var(--green);margin-top:2px">⏳ จองอยู่ อีก <span class="lock-countdown"></span></div>`:`<div style="font-size:11px;color:#ef4444;margin-top:2px">⚠️ หมดเวลาจอง ชำระเงินเร็วๆ</div>`;return `<div class="cart-item"><div class="cart-thumb">${x.image_url?`<img src="${imgSrc(x.image_url)}"/>`:(EMOJIS[x.category]||'📦')}</div><div class="cart-info"><div class="cart-name">${x.title}</div><div class="cart-price">฿${Number(x.price).toLocaleString()} × ${x.qty}</div>${lockHtml}</div><div class="cart-qty"><button class="btn btn-sm" onclick="changeQty(${x.product_id},${x.qty-1})">-</button><span>${x.qty}</span><button class="btn btn-sm" onclick="changeQty(${x.product_id},${x.qty+1})" disabled style="opacity:.3;cursor:not-allowed">+</button><button class="btn btn-sm btn-danger" onclick="removeCartItem(${x.product_id})">✕</button></div></div>`;}).join('')+`<div class="cart-summary"><div class="summary-row"><span>${items.length} รายการ</span><span>฿${total.toLocaleString()}</span></div><div class="summary-row"><span>ค่าจัดส่ง</span><span>ฟรี</span></div><div class="summary-total"><span>รวมทั้งหมด</span><span>฿${total.toLocaleString()}</span></div><button class="btn btn-g full" style="margin-top:16px" onclick="doCheckout()">ยืนยันชำระเงิน</button></div>`;goPage('cart');startLockCountdowns();}catch(e){toast(e.message);}
}
...
function profileTab(tab){
  ...
  if(tab==='products'){
    ...
    Promise.all([api.getMyProducts(), api.getIncomingOffers()]).then(([products, offers])=>{
      ...
      if(FEATURES.RESERVATION&&reserved.length){html+=`<div style="font-size:13px;font-weight:600;color:#b45309;margin:${available.length?'16px':'0px'} 0 8px;text-transform:uppercase;letter-spacing:.5px">🟡 กำลังถูกจอง (${reserved.length})</div>${reserved.map(renderItem).join('')}`;}
...
async function openMyFeedbackTab(){
  const c=document.getElementById('profileTabContent');
  c.innerHTML='<div class="loading">กำลังโหลด...</div>';
  const fbCatLabel={inquiry:'❓ สอบถาม',bug:'🐛 แจ้งบัค',feature:'💡 เสนอ Feature',complaint:'😤 ร้องเรียน',review:'💬 ติชม',keep:'⭐ ฟังก์ชั่นที่ดี',other:'📌 อื่นๆ'};
  const fbStatusColor={new:'#6366f1',reviewed:'#2563eb',resolved:'#16a34a'};
  const fbStatusLabel={new:'🆕 ใหม่',reviewed:'🔵 รับเรื่องแล้ว',resolved:'✅ แก้ไขแล้ว'};
  try{
    const fbs=await api.getMyFeedback();
    if(!fbs.length){
      c.innerHTML=`<div class="empty-state" style="margin-top:32px"><span class="empty-state-icon">📩</span><h3>ยังไม่มีประวัติ</h3><p>ส่งข้อความแรกได้เลย!</p><button class="btn btn-g" style="margin-top:12px" onclick="openFeedbackModal()">📩 ติดต่อแอดมิน</button></div>`;
      return;
    }
    c.innerHTML=`<div style="margin-top:16px">
      ...
      ${fbs.map(f=>`
        <div style="border:1px solid var(--border);border-left:3px solid ${fbStatusColor[f.status||'new']};border-radius:var(--radius);padding:14px;margin-bottom:10px;background:var(--card-bg)">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:6px">
            <span style="font-size:12px;font-weight:600;background:#eef2ff;color:#6366f1;padding:2px 8px;border-radius:99px">${fbCatLabel[f.category]||f.category}</span>
            <span style="font-size:12px;font-weight:600;color:${fbStatusColor[f.status||'new']}">${fbStatusLabel[f.status||'new']}</span>
          </div>
          <div style="font-size:11px;color:var(--text-hint);margin-bottom:8px;display:flex;flex-direction:column;gap:2px">
            <span>📅 ส่งเมื่อ: ${_fmtTime(f.created_at)}</span>
            ${f.updated_at?`<span>💬 แอดมินตอบ: ${_fmtTime(f.updated_at)}</span>`:''}
          </div>
          <div style="font-size:14px;color:var(--text);line-height:1.6;margin-bottom:8px">${f.message}</div>
          <div id="myFbThread_${f.id}" style="max-height:180px;overflow-y:auto;border:1px solid var(--border);border-radius:var(--radius);padding:8px;margin:8px 0;background:var(--bg)"><div style="font-size:12px;color:var(--text-hint);text-align:center">กำลังโหลด...</div></div>
          <div style="display:flex;gap:6px;margin-top:4px"><input type="text" id="myFbMsg_${f.id}" placeholder="ส่งข้อความเพิ่มเติม..." style="flex:1;padding:8px;border:1px solid var(--border);border-radius:var(--radius);background:var(--bg-sec);color:var(--text);font-size:13px" onkeydown="if(event.key==='Enter')userSendFeedbackMsg(${f.id})"/><button class="btn btn-sm btn-g" onclick="userSendFeedbackMsg(${f.id})">ส่ง</button></div>
        </div>`).join('')}
    </div>`;
    fbs.forEach(f=>_renderFeedbackThread(f.id,'#myFbThread_'+f.id,false));
  }catch(e){c.innerHTML=`<div style="color:#dc2626;padding:16px">${e.message}</div>`;}
}

function openInlineNameEdit(){
  const nameEl=document.getElementById('profileDisplayName');
  if(!nameEl)return;
  const pName=nameEl.closest('.p-name');
  pName.innerHTML=`<input id="inlineNameInput" value="${state.user.name}" style="font-size:15px;font-weight:600;border:1.5px solid var(--green);border-radius:6px;padding:4px 8px;width:160px;outline:none" onkeydown="if(event.key==='Enter')saveInlineName()"><button onclick="saveInlineName()" class="btn btn-g" style="font-size:12px;padding:4px 10px;margin-left:6px">บันทึก</button><button onclick="openProfile()" class="btn" style="font-size:12px;padding:4px 10px;margin-left:4px">ยกเลิก</button>`;
  const inp=document.getElementById('inlineNameInput');inp.focus();inp.select();
}
async function saveInlineName(){
  const input=document.getElementById('inlineNameInput');
  const newName=input?.value?.trim();
  if(!newName){toast('กรุณาระบุชื่อ');return;}
  try{
    await api.updateName(newName);
    state.user.name=newName;
    localStorage.setItem('user',JSON.stringify(state.user));
    const pName=input.closest('.p-name');
    const tier=state.user.shop_tier||'bronze';
    const tierLabel={bronze:'🥉 Bronze',silver:'🥈 Silver',gold:'🥇 Gold',diamond:'💎 Diamond'};
    pName.innerHTML=`<span id="profileDisplayName">${newName}</span><button onclick="openInlineNameEdit()" style="background:none;border:none;cursor:pointer;font-size:12px;padding:2px 4px;margin-left:2px;opacity:.5;line-height:1" title="แก้ไขชื่อ">✏️</button>${state.user.is_verified?'<span class="verified-badge" style="margin-left:6px">✅ Verified</span>':''} <span class="tier-badge tier-${tier}">${tierLabel[tier]}</span>`;
    updateNav();
    toast('เปลี่ยนชื่อแล้ว ✅','#1D9E75');
  }catch(e){toast(e.message);}
}
function openInlineShopNameEdit(){
  const shopNameEl=document.getElementById('profileShopName');
  if(!shopNameEl)return;
  const current=shopNameEl.textContent||'';
  const container=shopNameEl.parentElement;
  container.innerHTML=`<input id="inlineShopNameInput" value="${current}" placeholder="ชื่อร้านของคุณ" style="font-size:13px;border:1.5px solid var(--green);border-radius:6px;padding:3px 8px;width:160px;outline:none" onkeydown="if(event.key==='Enter')saveInlineShopName()"><button onclick="saveInlineShopName()" class="btn btn-g" style="font-size:12px;padding:3px 10px;margin-left:6px">บันทึก</button><button onclick="openProfile()" class="btn" style="font-size:12px;padding:3px 10px;margin-left:4px">ยกเลิก</button>`;
  const inp=document.getElementById('inlineShopNameInput');inp.focus();inp.select();
}
async function saveInlineShopName(){
  const input=document.getElementById('inlineShopNameInput');
  const newShopName=input?.value?.trim()||'';
  try{
    await api.updateShop({shop_name:newShopName,shop_bio:state.user?.shop_bio||''});
    if(state.user){state.user.shop_name=newShopName||null;localStorage.setItem('user',JSON.stringify(state.user));}
    const container=input.parentElement;
    container.innerHTML=`🏪 <span id="profileShopName">${newShopName}</span>${newShopName?`<button onclick="openInlineShopNameEdit()" style="background:none;border:none;cursor:pointer;font-size:12px;padding:2px 4px;margin-left:2px;opacity:.5;line-height:1" title="แก้ไขชื่อร้าน">✏️</button>`:`<button onclick="openInlineShopNameEdit()" style="background:none;border:none;cursor:pointer;font-size:12px;padding:2px 4px;opacity:.5;line-height:1;color:var(--green)">+ เพิ่มชื่อร้าน</button>`}`;
    toast('บันทึกชื่อร้านแล้ว ✅','#1D9E75');
  }catch(e){toast(e.message);}
}
...
async function toggleBlock(userId, isBlocked){
  ...
  try{
    if(isBlocked){
      ...
      toast('ยกเลิกการบล็อกแล้ว');
    }else{
      if(!confirm('บล็อกผู้ใช้นี้? พวกเขาจะไม่สามารถแชทหรือเสนอราคาสินค้าของคุณได้'))return;
      await api.blockUser(userId);
      toast('บล็อกผู้ใช้แล้ว 🚫');
    }
...
async function openSellerProfile(userId){try{const reqs=[api.getShop(userId),api.getSellerProducts(userId),api.getFollowerCount(userId),api.getSellerReviews(userId)];if(state.user)reqs.push(api.getFollowStatus(userId),api.getBlockStatus(userId));const[seller,products,followerData,reviewData,...rest]=await Promise.all(reqs);const followStatus=rest[0];const blockStatus=rest[1]||{};document.getElementById('sellerBackBtn').onclick=()=>history.back();const avatarHtml=seller.avatar?`<img src="${imgSrc(seller.avatar)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>`:`${(seller.name||'?').slice(0,2).toUpperCase()}`;const isOwnProfile=state.user&&state.user.id===seller.id;const isFollowing=followStatus?.following||false;const followerCount=followerData?.count||0;
...
const respMins=seller.avg_response_minutes;const respLabel=seller.response_count>=1?(respMins<60?'⚡ ตอบเร็วมาก':respMins<240?`🟢 ตอบใน ${Math.round(respMins/60)} ชม.`:respMins<1440?`🟡 ตอบใน ${Math.round(respMins/60)} ชม.`:`🔴 ตอบใน ${Math.round(respMins/1440)} วัน`):null;
document.getElementById('sellerContent').innerHTML=`${seller.shop_banner?`<div style="width:100%;height:140px;border-radius:var(--radius);overflow:hidden;margin-bottom:12px"><img src="${imgSrc(seller.shop_banner)}" style="width:100%;height:100%;object-fit:cover"/></div>`:''}<div class="profile-header"><div class="p-avatar" style="overflow:hidden">${avatarHtml}</div><div style="flex:1"><div class="p-name">${seller.shop_name||seller.name}${seller.shop_name?`<span style="font-size:12px;color:var(--text-sec);font-weight:normal;margin-left:6px">(${seller.name})</span>`:''}${seller.is_verified?'<span class="verified-badge">✅ ยืนยันแล้ว</span>':''}${respLabel?`<span style="font-size:12px;font-weight:400;margin-left:8px;color:var(--text-sec)">${respLabel}</span>`:''} <span class="tier-badge ${tierClass[sellerTier]}">${tierLabel[sellerTier]}</span></div><div class="p-email">สมาชิกตั้งแต่ ${new Date(seller.created_at).toLocaleDateString('th',{year:'numeric',month:'long'})}</div><div class="p-stats"><div class="stat"><div class="stat-n" style="font-size:20px">${products.length}</div><div class="stat-l">สินค้า</div></div><div class="stat"><div class="stat-n" style="font-size:20px">${seller.rating||5.0}★</div><div class="stat-l">คะแนน</div></div><div class="stat"><div class="stat-n" style="font-size:20px">${reviewData.count||0}</div><div class="stat-l">รีวิว</div></div><div class="stat"><div class="stat-n" style="font-size:20px">${followerCount}</div><div class="stat-l">ผู้ติดตาม</div></div></div>${!isOwnProfile?`<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap"><button class="btn btn-g" style="flex:1" onclick="startChat(${seller.id},null)">💬 แชทกับผู้ขาย</button><button class="btn ${isFollowing?'btn-danger':''}" style="flex:1" onclick="toggleFollow(${seller.id})">${isFollowing?'💔 เลิกติดตาม':'❤️ ติดตาม'}</button><button class="btn ${blockStatus.i_blocked?'btn-danger':''}" style="flex:1;min-width:120px" onclick="toggleBlock(${seller.id},${!!blockStatus.i_blocked})">${blockStatus.i_blocked?'🚫 เลิกบล็อก':'🚫 บล็อก'}</button></div>`:''}</div></div>${renderSellerStories(seller.id)}<div class="section-title" style="margin-top:20px"><span>สินค้าทั้งหมด (${publicProducts.length})</span></div><div class="product-grid" id="sellerProductGrid"></div><div class="reviews-section" style="margin-top:24px"><h3>รีวิวผู้ขาย (${reviewData.count}) — เฉลี่ย ${reviewData.average}★</h3>${reviewsHtml}</div>`;renderCards(publicProducts,'sellerProductGrid');goPage('seller');}catch(e){toast(e.message);}}
...
socket.on('feedback:new',({sender_name})=>{if(!state.user?.is_admin)return;toast(`📩 Feedback ใหม่จาก ${sender_name}`,'#6366f1');loadAdminFeedbackBadge();if(document.querySelector('.admin-tab.on')?.id==='atab-feedback')_reloadFeedbackTab();});
socket.on('feedback:new_message',({feedbackId,sender})=>{if(!state.user?.is_admin)return;toast(`💬 ${sender} ตอบกลับ Feedback`,'#2563eb');loadAdminFeedbackBadge();if(document.querySelector('.admin-tab.on')?.id==='atab-feedback'){_reloadFeedbackTab().then(()=>{const el=document.querySelector(`#fbThread_${feedbackId}`);if(el)_renderFeedbackThread(feedbackId,`#fbThread_${feedbackId}`,true);});}});
socket.on('feedback:update',({status,reply,feedbackId})=>{
  ...
  else toast('💬 แอดมินตอบกลับ Feedback แล้ว','#6366f1');
...
async function init(){
  ...
  socket?.on('live:list', () => { if(FEATURES.LIVE) loadLiveList(); });
...
function showPaymentFromOrder(id){
  ...
  if(!o){toast('กรุณาโหลดหน้าประวัติซื้อใหม่');return;}
...
async function _loadMoreProducts(){
  ...
  _productPage++;
  const q=document.getElementById('searchQ').value,minPrice=document.getElementById('minP').value,maxPrice=document.getElementById('maxP').value,sort=document.getElementById('sortSel').value,condition=document.getElementById('condSel').value,location=document.getElementById('locationSel').value;
  try{const params={page:_productPage,limit:_productLimit};if(state.cat!=='ทั้งหมด')params.cat=state.cat;if(q)params.q=q;if(minPrice)params.minPrice=minPrice;if(maxPrice)params.maxPrice=maxPrice;if(sort)params.sort=sort;if(condition)params.condition=condition;if(location)params.location=location;const products=await api.getProducts(params);if(!products.length){_productDone=true;}else{const g=document.getElementById('productGrid');const sentinel=document.getElementById('scrollSentinel');const frag=document.createDocumentFragment();products.forEach(p=>{const div=document.createElement('div');div.className='card';div.dataset.id=String(p.id);div.onclick=()=>openDetail(p.id);const priceHtml=p.original_price?`<div class="card-price">฿${Number(p.price).toLocaleString()} <span class="original-price">฿${Number(p.original_price).toLocaleString()}</span></div>`:`<div class="card-price">฿${Number(p.price).toLocaleString()}</div>`;const softLk=p.cart_locked_until&&new Date(p.cart_locked_until)>new Date()&&Number(p.cart_locked_by)!==Number(state.user?.id);const rBadge=p.status==='reserved'?'<span class="reserved-badge">รอยืนยัน</span>':softLk?'<span class="reserved-badge" style="background:rgba(251,191,36,.9)">⏳ กำลังถูกจอง</span>':'';div.innerHTML=`<div class="card-img">${productImg(p)}${p.original_price?'<span class="price-drop-badge">ลดราคา</span>':''}${rBadge}</div><div class="card-body"><div class="card-title">${p.title}</div>${priceHtml}<div class="card-foot"><span class="cond ${CMAP[p.condition||p.cond]||''}">${p.condition||p.cond}</span><span class="seller-nm">${p.seller_name||p.location||''}</span></div></div>`;frag.appendChild(div);});if(sentinel)g.insertBefore(frag,sentinel);else g.appendChild(frag);if(products.length<_productLimit)_productDone=true;}}
...
async function doSaveShop(){
  ...
  try{
    ...
    // update cached user
    if(state.user){
      if(profileName)state.user.name=profileName;
      state.user.shop_name=shopName||null;
      localStorage.setItem('user',JSON.stringify(state.user));
    }
    // refresh name elements if profile page is visible
    const nameEl=document.getElementById('profileDisplayName');
    if(nameEl&&profileName)nameEl.textContent=profileName;
    const shopNameEl=document.getElementById('profileShopName');
...
function _fmtTime(ts){if(!ts)return'';return new Date(ts).toLocaleString('th',{year:'numeric',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});}
async function _renderFeedbackThread(feedbackId,containerSelector,isAdmin){
  const msgs=await api.getFeedbackMessages(feedbackId).catch(()=>[]);
  const el=document.querySelector(containerSelector);
  if(!el)return;
  el.innerHTML=msgs.length?msgs.map(m=>`<div style="display:flex;flex-direction:column;align-items:${m.is_admin?'flex-end':'flex-start'};margin-bottom:8px"><div style="max-width:80%;background:${m.is_admin?'#eef2ff':'var(--bg-sec)'};color:var(--text);border-radius:12px;padding:8px 12px;font-size:13px;line-height:1.5">${m.message}</div><div style="font-size:10px;color:var(--text-hint);margin-top:2px;padding:0 4px">${m.is_admin?'🛡️ แอดมิน':'👤 คุณ'} · ${_fmtTime(m.created_at)}</div></div>`).join(''):'<div style="font-size:13px;color:var(--text-hint);text-align:center;padding:8px">ยังไม่มีข้อความ</div>';
...
async function adminSendFeedbackMsg(id){
  const el=document.getElementById('fbMsg_'+id);
  const msg=(el?.value||'').trim();
  if(!msg)return;
  try{await api.sendFeedbackMessage(id,msg);el.value='';await _renderFeedbackThread(id,'#fbThread_'+id,true);loadAdminFeedbackBadge();}catch(e){toast(e.message);}
}
async function userSendFeedbackMsg(id){
  const el=document.getElementById('myFbMsg_'+id);
  const msg=(el?.value||'').trim();
  if(!msg)return;
  try{await api.sendFeedbackMessage(id,msg);el.value='';await _renderFeedbackThread(id,'#myFbThread_'+id,false);}catch(e){toast(e.message);}
}
...
function openFeedbackModal(){
  ...
  const u=state.user;
  document.getElementById('feedbackName').value=u?.name||'';
  document.getElementById('feedbackEmail').value=u?.email||'';
...
async function doSubmitFeedback(){
  ...
  // Always use account email when logged in so history tab works
  const finalEmail=state.user?.email||sender_email||null;
  const finalName=sender_name||state.user?.name||null;
  try{
    const res=await api.submitFeedback(category,message,finalName,finalEmail);
...
function startLockCountdowns(){
  document.querySelectorAll('.lock-timer[data-lock-end]').forEach(el=>{
    if(el._lockTimer)return;
    el._lockTimer=true;
    const end=new Date(el.dataset.lockEnd);
    const span=el.querySelector('.lock-countdown');
    function tick(){
      if(!el.isConnected)return;
      const diff=end-Date.now();
      if(diff<=0){el.outerHTML='<div style="font-size:11px;color:#ef4444;margin-top:2px">⚠️ หมดเวลาจอง ชำระเงินเร็วๆ</div>';return;}
      const m=Math.floor(diff/60000),s=Math.floor((diff%60000)/1000);
      if(span)span.textContent=`${m}:${String(s).padStart(2,'0')} นาที`;
      setTimeout(tick,1000);
    }
    tick();
  });
...
async function addToCartFromLive(productId) {
  try {
    const res = await api.buyNow(productId);
    const popup = document.getElementById('liveProductPopup');
