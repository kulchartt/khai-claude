...
async function handleVerifyRequest(id,action){
  let note='';
  if(action==='reject'){
    note=window.prompt('เหตุผลที่ปฏิเสธ (ไม่บังคับ):','')??'';
  }
  try{
    const r=await api.adminHandleVerifyRequest(id,action,note||null);
    toast(r.message,action==='approve'?'#1D9E75':undefined);
