...
function openSell(){if(!state.user){toast('กรุณาเข้าสู่ระบบก่อน');openOverlay('loginOverlay');return;}_clearAccFiles('sImg');_meetupData.s=null;const si=document.getElementById('sMeetupInfo');if(si){si.style.display='none';si.textContent='';}const sDel=document.getElementById('sDel');if(sDel)sDel.value='both';openOverlay('sellOverlay');requestAnimationFrame(()=>toggleMeetupBtn('s'));}
