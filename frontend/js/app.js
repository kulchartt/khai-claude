...
async function doSaveShop(){
  ...
  try{
    ...
    // refresh name in profile page if visible
    const nameEl=document.getElementById('profileDisplayName');
    if(nameEl){
      const displayName=shopName||(profileName||state.user?.name||'');
      const realName=profileName||state.user?.name||'';
      nameEl.textContent=displayName;
      const realNameEl=nameEl.nextElementSibling;
      if(shopName&&realNameEl&&realNameEl.tagName==='SPAN')realNameEl.textContent=`(${realName})`;
    }
    updateNav();
