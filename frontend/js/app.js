      ...
      ${fbs.map(f=>`
        <div style="border:1px solid var(--border);border-left:3px solid ${fbStatusColor[f.status||'new']};border-radius:var(--radius);padding:14px;margin-bottom:10px;background:var(--card-bg)">
          <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px;flex-wrap:wrap;gap:6px">
            ...
            <span style="font-size:11px;color:var(--text-hint)">${new Date(f.created_at).toLocaleDateString('th',{year:'numeric',month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'})}</span>
          </div>
